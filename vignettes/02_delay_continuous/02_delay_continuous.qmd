...
discretised onto a mesh and lowered to a SciML `DDEProblem` for
solution with `DelayDiffEq.MethodOfSteps`.
...
We retain a small mesh so the matrices stay legible. Mass leaks out
of bin 1 into bin 2 with a fixed delay:
...
- `DelayGeneratorTerm(τ, H)` and `DelayIPMProblem` extend the
  generator API with arbitrary delay kernels.
